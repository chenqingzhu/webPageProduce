...
void print_lable_feature(struct lableFeature lf)
{
    ...
    cout << "\nlableContentLength:" <<lf.lableContentLength<<endl;
...
//获取左标签的class属性
string  get_left_lable_class_msg(string leftPartContent)
{
    int pos = leftPartContent.find("class=");
    if(pos == string::npos)
        return "NULL";
    int leftyinghao = leftPartContent.find("\"",pos+1);
    int rightyinghao = leftPartContent.find("\"",leftyinghao+1);
    return leftPartContent.substr(leftyinghao+1,rightyinghao-leftyinghao-1);
}

//获取左标签的id属性
string  get_left_lable_id_msg(string leftPartContent)
{
    int pos = leftPartContent.find("id=");
    if(pos == string::npos)
        return "NULL";
    int leftyinghao = leftPartContent.find("\"",pos+1);
    int rightyinghao = leftPartContent.find("\"",leftyinghao+1);
        ...
        if(msg.lableFlag == LEFTLABLE) //左标签
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(lableContent.find("All Rights Reserved") != string::npos || lableContent.find("【免责声明】") != string::npos || (lableContent.find("sohu.com")!= string::npos &&  lableContent.find("http://")!= string::npos ) || lableContent.find("all rights reserved") != string::npos ||(lableContent.find("网站运营：") != string::npos) ||((lableContent.find("法律事务：") != string::npos) && (lableContent.find("电子邮件：") != string::npos) ))
                        lableName = "a";
                    ...
                    topMsg = lableStack.top();
                    lableStack.pop();
                    lableMsg fatherMsg,fatherMsg2;
                    string fatherLableLeftPartContent;
                    string fatherClassMsg,fatherIdMsg;
                    string fatherLableName;
                    if (lableStack.empty()) {
                        fatherLableName ="NULL";
                        fatherClassMsg = "NULL";
                        fatherIdMsg = "NULL";
                    }
                    else
                    {
                        fatherMsg = lableStack.top();
                        fatherLableLeftPartContent = content.substr(fatherMsg.beginIndex,fatherMsg.endIndex - fatherMsg.beginIndex +1);
                        fatherClassMsg = get_left_lable_class_msg(fatherLableLeftPartContent);
                        fatherIdMsg = get_left_lable_id_msg(fatherLableLeftPartContent);
                        fatherLableName = fatherMsg.lableName;
                        if(fatherClassMsg== "NULL" && fatherIdMsg =="NULL")
                        {
                            lableStack.pop();
                            fatherMsg2 = lableStack.top();
                            fatherLableLeftPartContent = content.substr(fatherMsg2.beginIndex,fatherMsg2.endIndex - fatherMsg2.beginIndex +1);
                            fatherClassMsg = get_left_lable_class_msg(fatherLableLeftPartContent);
                            fatherIdMsg = get_left_lable_id_msg(fatherLableLeftPartContent);
                            fatherLableName = fatherMsg2.lableName;
                            lableStack.push(fatherMsg);
                        }
                    }
                    lf.fatherLableClassMessage = fatherClassMsg;
                    lf.fatherLableName = fatherLableName;
                    lf.fatherLableIdMessage = fatherIdMsg;
        ...
        else //右标签
        {
            ...
            if(topMsg.lableName == msg.lableName && topMsg.lableFlag == LEFTLABLE )
            {
                ...
                else
                {
                    ...
                    int lableLevelNumber = lableStack.size();
                    //网站信息定义为a标签，这样做：1、避免在聚类时候影响文本簇的选择，2、如果正文中有“All Rights Reserved”，也不会影响文本提取；
                    if(lableContent.find("All Rights Reserved") != string::npos || lableContent.find("【免责声明】") != string::npos || (lableContent.find("sohu.com")!= string::npos &&  lableContent.find("http://")!= string::npos ) ||(lableContent.find("网站运营：") != string::npos) ||((lableContent.find("法律事务：") != string::npos) && (lableContent.find("电子邮件：") != string::npos) ))
                        lableName = "a";
                    struct lableFeature lf(lableName,lableContent,lableLeftPartContent,lableBeginIndex,lableEndIndex,
                                           lableId++,lableContentLength,lableLeftPartLength,lableRightPartLength,lablePunctNumber,lableAttributeVector,lableLevelNumber);
                    ...
                    lf.fatherLableIdMessage = fatherIdMsg;
                    /*
                    lableMsg fatherMsg = lableStack.top();
                    string fatherLableLeftPartContent = content.substr(fatherMsg.beginIndex,fatherMsg.endIndex - fatherMsg.beginIndex +1);
                    string fatherClassMsg = get_left_lable_class_msg(fatherLableLeftPartContent);
                    lf.fatherLableClassMessage = fatherClassMsg;
                    lf.fatherLableName = fatherMsg.lableName;
                    */
                    if(lableName == "i")
                    {
                        if( lableContent.find("http") == 0 && ( (lableContent.find("jpg") == lableContent.size() - 3) ||(lableContent.find("JPG") == lableContent.size() - 3 ) ) )
...
vector<int> get_lable_content_information(vector<struct lableFeature>lf,vector<int> id,string selectLableName,vector<int> &lableDistance)
{
    ...
    int maxLengthId = id[0];
    for(int i=0;i<id.size();i++)
    {
        totalContentLen += lf[id[i]].lableContentLength;
        if(lf[maxLengthId].lableContentLength  < lf[id[i]].lableContentLength )
            maxLengthId = id[i];
...
string  print_page_content_by_id(vector<struct lableFeature>lf,vector<int> id,string selectLableName)
{
    ...
    //return  <0、所有标签文本平均长度，1、文本平均长度，2、文本标签数量，3、格式标签的个数，4、a标签的个数，5、h标签个数，6、div标签的个数,7、其他标签的个数,8、文本簇的中心数量,9、相邻标签的index距离大约40的个数,10、所有标签中具有最长contentLength的Id>
    vector<int> lableDistance;
    string tmpstring("新闻网页判断情况:\n");//新闻网页判断情况
    ...
    if(cluster_id_information_vect[1] < 33 && cluster_id_information_vect[9] >=4)
    {
        ...
        cout<<"文本平均长度小于33，且标签距离大约40的个数大于等于4个，判断是非新闻网页"<<endl;
        tmpstring +="\t\t文本平均长度小于33，且标签距离大约40的个数大于等于4个，判断是非新闻网页\n";
    }
     /*
    ...
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t h 标签的个数大约等于3，且总标签的平均长度小于60，非新闻网页\n";
        cout<<"h 标签的个数大约等于3，且总标签的平均长度小于60，非新闻网页"<<endl;
    }
    if(cluster_id_information_vect[5] >=6)
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t h 标签的个数大约等于6，非新闻网页\n";
        cout<<"h 标签的个数大约等于6,非新闻网页"<<endl;
    }
    if(cluster_id_information_vect[4] >= id.size() * 1 / 2)
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t a 标签的个数大于一半，非新闻网页 \n";
        cout<<"a 标签的个数大于一半，非新闻网页。标签总数："<<id.size() <<"  a标签数量："<<cluster_id_information_vect[4]<<endl;
    }
    if( cluster_id_information_vect[9] > lableDistance.size() - cluster_id_information_vect[9] && id.size() >2)
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t 标签之间距离大约40的个数大于标签距离小于40的个数，非新闻网页\n";
        cout<<"标签之间距离大约40的个数大于标签距离小于40的个数，非新闻网页"<<endl;
    }
    if(cluster_id_information_vect[1] > 150 && id.size() <= 4 && if_not_is_a_news_web_page_flag)
    {
        if_not_is_a_news_web_page_flag = false;
        tmpstring +="\t\t平均文本距离大于150，且标签个数小于等于4个，是新闻网页\n";
        cout<<"平均文本距离大于150，且标签个数小于等于4个，是新闻网页"<<endl;
    }
    
    if(cluster_id_information_vect[1] < 30)
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t平均文本距离小于30，非新闻网页\n";
        cout<<"平均文本距离小于30，非新闻网页"<<endl;
    }
    
    if(cluster_id_information_vect[2] < 5 && cluster_id_information_vect[1] < 60 )
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t平均文本距离小于60，且文本标签个数的小于5个，非新闻网页\n";
        cout<<"平均文本距离小于60，且文本标签个数的小于5个，非新闻网页"<<endl;
    }
    
    //对于单句新闻网页的判断
    if(lf[cluster_id_information_vect[10]].lableContentLength >= 240 && (!(lf.size() >= 400 && cluster_id_information_vect[10] >= lf.size() * 92/100)) && if_not_is_a_news_web_page_flag)
    {
        if_not_is_a_news_web_page_flag = false;
        tmpstring +="\t\t最长的标签文本长度大于240，则可定为单句新闻网页，是新闻网页\n";
        cout<<"最长的标签文本长度大于240，则可定为单句新闻网页，是新闻网页"<<endl;
    }
    
    if(cluster_id_information_vect[5] > id.size() *1/2 )
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t h 标签的个数大于一半，非新闻网页\n";
        cout<<"h 标签的个数大于一半，非新闻网页"<<endl;
    }
    
    if(cluster_id_information_vect[9] > 12 ||  cluster_id_information_vect[9] > id.size()*3/5)
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t不连续标签个数大于12，或者不连续标签数大于总标签的2/3，非新闻网页\n";
        cout<<"不连续标签个数大于12，或者不连续标签数大于总标签的2/3，非新闻网页"<<endl;
    }
    
    if(cluster_id_information_vect[8] == id.size() || cluster_id_information_vect[8] >=4 )
    {
        if_not_is_a_news_web_page_flag = true;
        tmpstring +="\t\t文本中心数等于标签数量，或者文本中心数大约等于4个，非新闻网页\n";
        cout<<"文本中心数等于标签数量，或者文本中心数大约等于4个，非新闻网页"<<endl;
    }
    
    if(id.size() == 1 && lf[id[0]].lableContentLength > 50)
    {
        if_not_is_a_news_web_page_flag = false;
        tmpstring +="\t\t只有一个标签，是新闻网页\n";
        cout<<"只有一个标签，是新闻网页"<<endl;
    }

    //有些图片描述会出现连续性重复（且lableID连续），要去除
    ...
    for(int i=0;i<id.size(); i++)
    {
        ...
        cout<<"--fatherLableName:"<<lf[id[i]].fatherLableName<<"--fatherlableClassMsg:"<<lf[id[i]].fatherLableClassMessage;
        cout<<"--fatherIdMsg:"<<lf[id[i]].fatherLableIdMessage;
    ...
    if(lf.size() >= 1600 && if_not_is_a_news_web_page_flag==false)
        page_text +="\n\n\n经判断为此网页的文本标签数大于1600，其可能不是新闻网页\n\n";
...
vector<int> get_include_lableid_by_a_lableId(vector<struct lableFeature>lf,int id)
{
    ...
    {
        //搜狐网页处理
        if(lf[i].lableContent.find("sohu.com")!= string::npos &&  lf[i].lableContent.find("http://")!= string::npos)
        {
            endID = i-1;
            break;
        }
        ...
        if(lf[i].lableContent.find("All Rights Reserved") !=string::npos && (i >=lf.size()* 95/100 || i >= lf.size() - 5) )
        {
            endID = i-1;
            break;
        }
        ...
        if(lf[i].lableName == "p" && lf[i].lableContentLength >= 300)
        {
            endID = i;
            last_end = endID;
            continue;
    ...
    {
        //cout<<"lableName:"<< lableName<<"    name:"<<lf[ret[i]].lableName<<endl;
        if(lf[ret[i]].lableName == lableName || (lf[ret[i]].lableName =="p" && lf[ret[i]].lableContentLength >= 300))
        {
            if(ret.size()-1 -i >=2)
            {
                ret.erase(ret.begin()+i+1,ret.end());
                cout<<"erase1: "<<*(ret.begin()+i+1) <<" to  "<< ret[ret.size()-1]<<endl;
                break;
            }
            break;
        }
    ...
    for(int j=ret.size()-1;j>=0;j--)
    {
        ...
        else
        {
            ...
            cout<<"erase2: "<<*(ret.begin()+j+1) <<" to  "<< ret[ret.size()-1]<<endl;
            break;
        }
    }
    ...
    for(int i=0;i<ret.size();i++)
    {
        ...
        cout<<"erase3: "<<ret[0] <<" to  "<< ret[i-1]<<endl;
        break;
    }
    cout<<"ret_id:";
    for(int i=0;i<ret.size();i++)
    ...
    else
    {
        ...
        if(each_center_id_vect[len_center-1][0] >= lf.size() * 91/100 ||(each_center_id_vect[len_center-1][1] >=lf.size()-3 && center_len<=4))
        {
             ...
             */
            {
                ...
                //当标签文本很短或者总标签数很大，正文却在尾部，这判断不是正文中心。
...
int main_loop_url_AGENS(string url,int kcluster = 5)
{
    ...
    try
    {
        ...
        //直接根据聚类中心选择文本簇
        int page_text_cluster_k_1 = kmeans.get_page_text_cluster_k();
        //根据各个簇中的top3进行选择
        int page_text_cluster_k = kmeans.get_page_text_cluster_k_1224();
        if(page_text_cluster_k != page_text_cluster_k_1)
        {
            cout<<"let's see\n";
        }
...
//读取配置文件  url对应的html文件路径
map<string,string> get_url_2_html_file_map(string filepath)
{
    map<string,string> url_dict;
    if(filepath.size() == 0)
        return url_dict;
    ifstream infile(filepath);
    if(!infile)
    {
        cout<<"get url 2 html file map ,open file error"<<endl;
        return url_dict;
    }
    string line;
    while(getline(infile, line))
    {
        //cout<<" line:"<<line<<endl;
        string url,filepath;
        int splitIndex = line.find("#3#");
        url = line.substr(0,splitIndex);
        filepath = line.substr(splitIndex+3);
        filepath.substr(1,filepath.size()-1);
        url_dict[url] = filepath;
        //cout<<"   url:"<<url << "  filePath:"<<filepath<<endl;
    }
    cout<<url_dict.size()<<endl;
    return url_dict;
}
void testFunction(string path,bool timeTest = false)
{
    map<string,string> url_dict = get_url_2_html_file_map(path);
    map<string,string>::iterator iter = url_dict.begin();
    int total_time = url_dict.size();
    int loop_time =0;
    cout<<"begin index: ";
    cin >> loop_time;
    int i=0;
    if(loop_time >= total_time)
        return;
    while(i++<loop_time)
    {
        iter ++;
    ...
    cout<<"begin: "<<ctime(&tm_begin)<<endl;

    while(iter!= url_dict.end())
    {
        main_loop_file_AGENS(iter->second,3);
        if(timeTest)
        {
        ...
        cout<<"file path:"<<iter->second<<endl;
        cout<<"tmp url: "<<iter->first<<endl;
        cout<<"loop_time:"<<loop_time<<endl;
        string next;
        cout<<"总个数："<< total_time <<"   输入下一个url坐标：";
        cin >> next;
...
int main()
{
    testFunction("/Users/pc/UrlFile/url_dict_file.txt");
    //return 1;
    //testUrlClass tuc;
